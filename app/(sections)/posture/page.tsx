    ...
    <div className="min-h-screen bg-[var(--background)] pb-24">
      ...
      <motion.div className="px-4 py-4 space-y-4" variants={container} initial="hidden" animate="show">
        ...
        {POSTURE_DATA.issues.map((iss, i) => (
          <motion.div key={i} variants={item} className="rounded-xl p-4 card-dark border" style={{ borderColor: iss.color + "40" }}>
            ...
            {iss.fixes.map((f, j) => (
              <div key={j} className="text-[11px] text-[var(--foreground)] py-1.5 leading-relaxed" style={{ borderBottom: j < iss.fixes.length - 1 ? "1px solid var(--theme-border)" : "none" }}>→ {f}</div>
            ))}
        ...
        <motion.div variants={item}>
          <div className="font-display text-xl text-[var(--theme-orange)] tracking-wider mb-1">⏱️ DAILY 10-MIN POSTURE ROUTINE</div>
        ...
        <motion.div variants={item} className="rounded-xl p-4 card-dark border border-[var(--theme-orange)]/30">
          {POSTURE_DATA.dailyRoutine.map((r, i) => (
            <div key={i} className="flex items-center justify-between py-2.5" style={{ borderBottom: i < 5 ? "1px solid var(--theme-border)" : "none" }}>
              <div>
              ...
              <div className="px-2 py-1 rounded bg-[var(--theme-dim)] text-[10px] text-[var(--theme-orange)] whitespace-nowrap">{r.time}</div>
            </div>
          ))}
        </motion.div>
        ...
        {POSTURE_DATA.habits.map((h, i) => (
          <motion.div key={i} variants={item} className="rounded-lg p-3 card-dark border-l-2 text-[12px] text-[var(--muted-foreground)] leading-relaxed" style={{ borderColor: "var(--cyber-blue)" }}>→ {h}</motion.div>
        ))}
      </motion.div>
      <BottomNav />
    </div>
