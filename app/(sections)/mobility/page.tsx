...
const TABS = [
  { key: "morning",     label: "🌅 Morning",      color: "var(--theme-accent-dark)" },
  { key: "preworkout",  label: "⚡ Pre-Workout",   color: "var(--theme-red)" },
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] pb-24">
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div className="rounded-xl p-3 card-dark text-[11px] text-[var(--muted-foreground)] leading-relaxed">
          <span className="text-[var(--theme-orange)] font-bold">RULE: </span>
        ...
        <div className="grid grid-cols-2 gap-2">
          {TABS.map((t) => (
            <button key={t.key} onClick={() => setTab(t.key)}
              ...
              style={{
                backgroundColor: tab === t.key ? t.color + "15" : "var(--theme-card)",
                borderColor: tab === t.key ? t.color + "60" : "var(--theme-border)",
        ...
        <motion.div key={tab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          className="rounded-xl p-4 card-dark border" style={{ borderColor: tabConfig.color + "40" }}>
          ...
          {section.moves.map((m, i) => (
            <div key={i} className="flex items-center justify-between py-2.5" style={{ borderBottom: i < section.moves.length - 1 ? "1px solid var(--theme-border)" : "none" }}>
              <div>
              ...
              <div className="px-2 py-1 rounded bg-[var(--theme-dim)] text-[10px] whitespace-nowrap" style={{ color: tabConfig.color }}>{m.reps}</div>
            </div>
          ))}
        </motion.div>
      </div>
